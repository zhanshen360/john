...
static struct opt_entry opt_list[] = {
	...
	{"status", FLG_STATUS_SET, FLG_STATUS_CHK, 0, ~FLG_STATUS_SET & OPT_ALL_FLAGS & ~FLG_DYNFMT, OPT_FMT_STR_ALLOC,
		&options.session},
	...
	{"test-full", FLG_TEST_SET, FLG_TEST_CHK,
		0, ~FLG_TEST_SET & ~FLG_FORMAT & ~FLG_SAVEMEM & ~FLG_DYNFMT & OPT_REQ_PARAM,
		"%d", &benchmark_level},
#ifdef HAVE_FUZZ
	{"fuzz", FLG_FUZZ_SET, FLG_FUZZ_CHK,
		0, ~FLG_FUZZ_DUMP_SET & ~FLG_FUZZ_SET & ~FLG_FORMAT & ~FLG_SAVEMEM & ~FLG_DYNFMT & OPT_ALL_FLAGS & ~FLG_NOLOG,
